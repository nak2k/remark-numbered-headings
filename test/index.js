/**
 * @typedef {import('remark-numbered-headings').Options} Options
 */
...
import remarkNumberedHeadings from 'remark-numbered-headings'

test('remarkToc', async function (t) {
  await t.test('should expose the public api', async function () {
    assert.deepEqual(Object.keys(await import('remark-numbered-headings')).sort(), [
      'default'
    ])
  })
})
...
test('fixtures', async function (t) {
  ...
  while (++index < folders.length) {
    ...
    await t.test(folder, async function () {
      ...
      const proc = remark().use(remarkNumberedHeadings, config)
